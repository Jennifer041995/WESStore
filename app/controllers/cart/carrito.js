$(function() {
    function renderCarrito(items) {
        ...
        const rows = items.map(p => {
            ...
            return `
                <tr>
                    <td>
                        <img src="${p.imagen_principal||'media/img/default.png'}" width="50" class="mr-2">
                        ${p.nombre}
                    </td>
    ...
    // Esta función carga el carrito desde el backend y lo renderiza
    ...
    // Eventos delegados para interacciones dentro del carrito
    $('#contenido-carrito')
    ...
    // Evento para abrir el modal de checkout y cargar el fragmento HTML
    $(document).on('click', '#finalizar-compra', function() {
        $('#checkout-container').load('app/views/checkout.html', function(resp, status) {
            ...
            // Asegúrate de que initCheckout() esté definido en checkout.js y cargado
            if (typeof initCheckout === 'function') {
                initCheckout();
            } else {
                console.error("La función initCheckout() no está disponible.");
                Swal.fire('Error', 'Problema al iniciar el proceso de pago. Intenta de nuevo.', 'error');
            }
            $('#checkoutModal').modal('show');
        });
    });

    // >>>>> NUEVO: Escuchar el evento 'carritoVaciado' y recargar el carrito <<<<<
    $(document).on('carritoVaciado', function() {
        console.log("Evento 'carritoVaciado' recibido en carrito.js. Recargando carrito...");
        cargarCarrito();
        // Opcional: Si el modal del carrito principal (no el de checkout) estuviera abierto,
        // podrías cerrarlo aquí para reflejar el carrito vacío.
        // Ejemplo (ajusta el ID del modal si es diferente):
        // $('#tuIdDelCarritoModal').modal('hide');
    });
